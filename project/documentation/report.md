...
| 2 | Multiple readers try to read from a non zero size buffer | Reading will be performed correctly. If read `n` times, it will receive `n` items. | --- | With this test we want to prove that reading in a multi-threaded environment works correctly. | see test_2 in the code|
